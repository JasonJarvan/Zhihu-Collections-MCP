...
async def export_collection_handler(args: dict) -> list[TextContent]:
    ...
    if output_path:
        main_module.base_output_path = str(output_path)
    else:
        main_module.base_output_path = None
    ...
    # 执行导出
    try:
        main_module.process_single_collection(collection_name, collection_url)
        result += "✅ 导出完成！"
    except Exception as e:
        result += f"❌ 导出失败: {str(e)}"
...
async def get_collection_info_handler(args: dict) -> list[TextContent]:
    ...
    try:
        collection_id = collection_url.split("?")[0].split("/")[-1]
        urls, titles = main_module.get_article_urls_in_collection(collection_id)
...
async def remove_from_collection_handler(args: dict) -> list[TextContent]:
